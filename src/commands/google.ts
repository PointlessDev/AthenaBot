...
import {GOOGLE_CSE_CX, GOOGLE_CSE_KEY} from '../config';
...
export default class SearchCommands {
  ...
  public async google(message: Message, args: Arguments): Promise<any> {
    let query = args.contentFrom(1);
    if(!query) return this.responder.fail(message, 'No query provided!');
    ...
    snekfetch.get(GOOGLE_ENDPOINT + safesearch + '&q=' + encodeURIComponent(query)).then((res: SnekResponse) => {
      ...
      message.channel.send({embed: {
        ...
        footer: {
          ...
          text: `Result 1 of ${parseInt(resultCount).toLocaleString()} | Results for search "${args.contentFrom(1)}"`
        },
        title: result.title,
        url: result.link
